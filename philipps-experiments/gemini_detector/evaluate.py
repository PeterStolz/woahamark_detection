"""Evaluate gemini_detector on a chosen split. Threshold tuned on val only."""
...
def score_split(items: List[Tuple[str, int]], score_fn) -> Tuple[List[float], List[int], List[str]]:
    scores, labels, paths = [], [], []
    for path, label in tqdm(items, desc="scoring"):
        img = cv2.imread(path, cv2.IMREAD_COLOR)
        scores.append(float(score_fn(img)) if img is not None else 0.0)
        labels.append(int(label))
        paths.append(path)
    return scores, labels, paths


def best_threshold_macro_f1(scores, labels, lo=0.05, hi=0.95, step=0.02):
...
def make_montage(paths_scores, max_n=32, cols=8, tile=160):
...
def save_failure_montages(paths, labels, scores, threshold, out_dir: Path, tag: str):
    out_dir.mkdir(parents=True, exist_ok=True)
    s = np.asarray(scores); y = np.asarray(labels)
    pred = (s >= threshold).astype(int)
    fn_idx = np.where((y == 1) & (pred == 0))[0]
    fn_items = [(paths[i], float(s[i])) for i in sorted(fn_idx.tolist(), key=lambda i: s[i])]
    fp_idx = np.where((y == 0) & (pred == 1))[0]
    fp_items = [(paths[i], float(s[i])) for i in sorted(fp_idx.tolist(), key=lambda i: -s[i])]
    if fn_items:
...
def main():
    ...
    ap.add_argument("--baseline", action="store_true",
                    help="Use the spec'd Phase 1 baseline (master template, wide scales).")
    args = ap.parse_args()
    ...
    if args.threshold is None:
        ...
        thr, val_f1 = best_threshold_macro_f1(val_scores, val_labels,
                                               lo=args.sweep_lo, hi=args.sweep_hi)
