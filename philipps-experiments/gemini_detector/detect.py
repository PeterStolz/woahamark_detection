"""Single-image scoring for the Gemini sparkle watermark.

Same Phase 3 knobs as grok_detector.detect (high-pass, distance-transform,
multi-template, configurable scales). Standalone — no import from grok_detector.
"""
...
def high_pass(gray: np.ndarray, sigma: float = 3.0) -> np.ndarray:
    ...
    return np.clip((f - blur) + 128.0, 0, 255).astype(np.uint8)
...
class ScoreConfig:
    ...
    high_pass_sigma: Optional[float] = None
    use_distance_transform: bool = False
    extra_templates: List[np.ndarray] = field(default_factory=list)
...
def _score_template(roi_edges, tpl_edges, scales, use_distance_transform: bool) -> float:
...
def score(image_bgr: np.ndarray, template_gray: np.ndarray,
    ...
    templates = [template_gray] + list(cfg.extra_templates)
    best = 0.0
    for tg in templates:
...
def classify(image_bgr, template_gray, threshold: float, cfg=None):
    s = score(image_bgr, template_gray, cfg)
    return s, int(s >= threshold)


# ---------- locked default (filled in after Phase 3) ----------

DEFAULT_THRESHOLD = 0.41          # Phase 3 winner: V5_realonly_narrow on val
DEFAULT_NARROW_SCALES = (0.85, 0.92, 1.0, 1.08, 1.15)


def default_config_and_template():
    """Returns (cfg, primary_template) for the locked configuration.
    Falls back to the master template before Phase 3 lands real-sample crops."""
    ...
    if paths:
        ...
        primary, cfg = load_template_gray(), ScoreConfig()
    return cfg, primary
