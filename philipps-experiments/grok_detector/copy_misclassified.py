"""Copy every misclassified image into debug/full_eval/{false_negatives,false_positives}/.

Reads debug/full_eval/all_scores.csv (produced by eval_full.py).
Filenames are prefixed with the score and split so they sort by confidence."""
...
def main():
    ...
    for d in (fn_dir, fp_dir):
        # wipe and recreate so reruns don't accumulate stale files
        if d.exists():
            shutil.rmtree(d)
    ...
    # rank FN by lowest score (worst first), FP by highest score (worst first)
    fn_rows.sort(key=lambda r: float(r["score"]))
    fp_rows.sort(key=lambda r: -float(r["score"]))

    def copy_one(r, idx, dst_dir, score_descending: bool):
        src = Path(r["path"])
        sc = float(r["score"])
        # zero-padded index keeps OS file listings sorted by rank
        prefix = f"{idx:03d}_s{sc:.3f}_{r['split']}"
        # sanitize: original filename may contain encoded URL characters
        safe_name = src.name.replace("/", "_")
        dst = dst_dir / f"{prefix}_{safe_name}"
        # truncate names that exceed common filesystem limits
        if len(dst.name) > 200:
    ...
    for i, r in enumerate(fn_rows):
        copy_one(r, i, fn_dir, score_descending=False)

    print(f"copying {len(fp_rows)} false positives -> {fp_dir}")
    for i, r in enumerate(fp_rows):
        copy_one(r, i, fp_dir, score_descending=True)

    # also write a small index csv per folder for context
    ...
    print(f"wrote {fn_dir/'_index.csv'} and {fp_dir/'_index.csv'}")
