"""Evaluate the locked V8 config on the full watermark_grok and no_watermark folders.

Threshold stays at the val-tuned DEFAULT_THRESHOLD; nothing is re-tuned here.
This is purely a sanity check across all 897 images, NOT a model-selection run.
"""
...
def main():
    ...
    # Identify which split each image came from (so we can report a within-split breakdown)
    split = load_split()
    split_of = {}
    ...
    for path, lab in tqdm(items, desc="full"):
        ...
        s = float(score(img, template, cfg)) if img is not None else 0.0
        paths.append(path); labels.append(lab); scores.append(s)
    ...
    report(np.ones_like(y, dtype=bool), "FULL (all 897 images)")
    for split_name in ("train", "val", "test"):
        mask = np.array([split_of.get(p) == split_name for p in paths])
        report(mask, f"split={split_name}")

    # Save full scores csv + montages of all FN / top-FP across the full dataset
