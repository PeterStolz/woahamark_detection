"""Build a single showcase image: best/worst positive & negative examples,
each shown as ROI alongside its Canny edges (the input the matcher actually sees)."""
...
def load_test_scores(csv_path: Path):
...
def build_panel(title: str, rows, threshold: float, n: int = 6):
    """Each row in `rows` has keys path, label, score, prediction.
    Renders a panel: title row + N items (ROI | Canny) with a per-cell label."""
    items = rows[:n]
    cell_w = TILE_W * 2 + GAP
    ...
    for i, r in enumerate(items):
        ...
        cell = np.concatenate(
            [roi_img,
             np.full((TILE_H, GAP, 3), 60, np.uint8),
             edge_img], axis=1)
        ...
        cls = "POS" if label == 1 else "NEG"
        verdict = "[OK]" if pred == label else "[MISS]"
        cv2.putText(panel, f"{cls} pred={'POS' if pred else 'NEG'} {verdict}  score={score:.3f}",
                    (x0 + 4, y0 + TILE_H + 18),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 230, 255), 1, cv2.LINE_AA)
        name = Path(r["path"]).name[:36]
...
def stack_panels(panels):
    """Vertically concatenate panels with a small gap, padding to common width."""
    width = max(p.shape[1] for p in panels)
    padded = []
    for p in panels:
        if p.shape[1] < width:
            pad = np.full((p.shape[0], width - p.shape[1], 3), 28, np.uint8)
            p = np.concatenate([p, pad], axis=1)
        padded.append(p)
        padded.append(np.full((PANEL_GAP, width, 3), 18, np.uint8))
...
def main():
    ...
    rows = load_test_scores(Path(args.scores_csv))
    pos = [r for r in rows if int(r["label"]) == 1]
    neg = [r for r in rows if int(r["label"]) == 0]

    # Best fake (highest TP scores)
    best_pos = sorted([r for r in pos if int(r["prediction"]) == 1],
                      key=lambda r: -float(r["score"]))
    # Worst fake (lowest scores among positives — includes FNs)
    worst_pos = sorted(pos, key=lambda r: float(r["score"]))
    # Best real (lowest TN scores — most confidently "no watermark")
    best_neg = sorted([r for r in neg if int(r["prediction"]) == 0],
                      key=lambda r: float(r["score"]))
    # Worst real (highest scores among negatives — closest to FP territory)
    worst_neg = sorted(neg, key=lambda r: -float(r["score"]))

    panels = [
        build_panel("BEST fake (Grok) - highest-confidence true positives",
                    best_pos, args.threshold, args.n),
        build_panel("WORST fake (Grok) - lowest scores on label=1 (false negatives if pred=NEG)",
